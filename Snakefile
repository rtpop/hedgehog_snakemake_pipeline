...
# SiSaNA directories
SISANA_DIR = os.path.join(OUTPUT_DIR, CANCER, "sisana")
...
# ELAND directories
ELAND_DIR = os.path.join(OUTPUT_DIR, CANCER, "eland")
...
ANALYSIS_DIR = os.path.join(OUTPUT_DIR, CANCER, "analysis")
...
rule benchmark_filtering:
    ...
    shell:
        """
        python {params.script} --filtered_net {input.panda_network_filtered} --prior_file {input.updated_prior_sep} --panda_edgelist {input.panda_edgelist} --output_file {output.filtering_bench} --delimiter {params.delimiter}
        """
